...
@with_kw struct Pars
    ...
    κ::Float64 = 0.00         #Fraction of debt recovered when default is resolved
    ...
    KeepRentsInDefault::Bool = false   #Sold rents are given up only when the government repays
    ...
    μ::Float64 = 2.9           #Armington elasticity, final sector (Mendoza and Yue(2012))
    μo::Float64 = μ            #Armington elasticity, oil sector. Same as the final
                               #sector's unless a variant moves it. Reconstructing Pars
                               #does not re-evaluate this default, so a variant that
                               #changes μ must set μo by hand, as αN does with αMo
    ...
    ν::Float64 = 2.44         #Elasticity across imported varieties, final sector
                              #(Mendoza and Yue(2012))
    νo::Float64 = ν           #Same, oil sector. As with μo, reconstructing Pars does not
                              #re-evaluate this default, so a variant that moves ν must
                              #set νo by hand
...
@with_kw mutable struct Solution{T1,T2,T3,T4}
    ### Arrays
    #Value Functions
    VD::T1
    VP::T1
...
function Compute_Quantities_GivenState(x::State,par::Pars)
    ...
    if IsEndowmentCase
        ...
        @unpack poil, r_star, pj, θo, θf, ν, νo = par
        @unpack z, n = x
        ...
        mdo=DomesticMaterialsDemand_h(false,x,w,pd,par)
        gdp_final=y-Pstar_f*mstar_f+w*Lo+pd*mdo
        gdp_oil=poil*yo-Pstar_o*mstar_o-w*Lo-pd*mdo

        #Also compute working capital
        mj_θf=((Pstar_f/((1+r_star)*pj))^ν)*mstar_f
        mj_θo=((Pstar_o/((1+r_star)*pj))^νo)*mstar_o
...
function Oil_Share_In_GDP_Average(n::Float64,par::Pars)
    @unpack μ_z, IsEndowmentCase = par
    Default=false; n_ind=1; z=μ_z; b=0.0
    x=State(Default,n_ind,n,z,b)
    gdp_final, gdp_oil, L, y, yo, WorkingCapital=Compute_Quantities_GivenState(x,par)
    if IsEndowmentCase
        #No production side, gdp_oil is the endowment itself
        return gdp_oil/(gdp_final+gdp_oil)
    else
        #The target is oil's share of GDP in the national accounts, which is value
        #added: the firm's operating surplus gdp_oil plus its wage bill
        pd=pd_Given_State(x,par)
        w=Wage_Given_pd(pd,x,par)
        Lo=LaborDemand_h(false,x,w,pd,par)
        return (gdp_oil+w*Lo)/(gdp_final+gdp_oil)
    end
end
...
################################################################
function CreateInterpolation_ValueFunctions(MAT::Array{Float64},GRIDS::Grids)
    #Value functions in default now carry b, re-entry is at κ*b
...
function InitiateSolution(GRIDS::Grids,par::Pars)
    ...
    VD=zeros(Float64,Nb,Nz,Nn)
    VP=zeros(Float64,Nb,Nz,Nn)
    V=zeros(Float64,Nb,Nz,Nn)
    itp_VD=CreateInterpolation_ValueFunctions(VD,GRIDS)
    itp_VP=CreateInterpolation_ValueFunctions(VP,GRIDS)
    itp_V=CreateInterpolation_ValueFunctions(V,GRIDS)
    #Expectations and prices
    EVD=zeros(Float64,Nb,Nz,Nn)
    EV=zeros(Float64,Nb,Nz,Nn)
    q1=zeros(Float64,Nb,Nz,Nn)
    qD=zeros(Float64,Nb,Nz,Nn)

    itp_EVD=CreateInterpolation_ValueFunctions(EVD,GRIDS)
    itp_EV=CreateInterpolation_ValueFunctions(EV,GRIDS)
    itp_q1=CreateInterpolation_Price(q1,GRIDS)
    itp_qD=CreateInterpolation_Price(qD,GRIDS)
    ...
    return Solution(VD,VP,V,EVD,EV,q1,qD,bprime,itp_VD,itp_VP,itp_V,itp_EVD,itp_EV,itp_q1,itp_qD,itp_bprime)
end
...
function StackSolution_Vector(SOLUTION::Solution)
    ...
    @unpack VD, EVD, qD = SOLUTION
    VEC=vcat(VEC,reshape(VD,(:)))
    VEC=vcat(VEC,reshape(EVD,(:)))
    VEC=vcat(VEC,reshape(qD,(:)))
...
function VectorOfRelevantParameters(par::Pars)
    ...
    VEC=vcat(VEC,par.κ)       #27

    #Sectoral technology, for variants that move it away from the Mexican matrix
    VEC=vcat(VEC,par.αMo)     #28
    VEC=vcat(VEC,par.λo)      #29
    VEC=vcat(VEC,par.λf)      #30
    VEC=vcat(VEC,par.μ)       #31
    VEC=vcat(VEC,par.μo)      #32
    VEC=vcat(VEC,par.ν)       #33
    VEC=vcat(VEC,par.νo)      #34
    VEC=vcat(VEC,par.αMf)     #35
...
function ExtractMatrixFromSolutionVector(start::Int64,size::Int64,VEC::Vector{Float64},GRIDS::Grids,par::Pars)
    @unpack Nz, Nb = par
    @unpack GR_n = GRIDS
    Nn=length(GR_n)
    finish=start+size-1
...
function TransformVectorToSolution(VEC::Array{Float64},GRIDS::Grids,par::Pars)
    ...
    #Repayment
    start=1
    VP=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)
    start=start+size_array
    V=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)
    start=start+size_array
    EV=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)
    start=start+size_array
    q1=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)
    start=start+size_array
    bprime=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)

    #Default
    start=start+size_array
    VD=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)
    start=start+size_array
    EVD=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)
    start=start+size_array
    qD=ExtractMatrixFromSolutionVector(start,size_array,VEC,GRIDS,par)
...
function UnpackParameters_Vector(VEC::Array{Float64})
    ...
    #Slots 28 to 35 are optional: model files saved before each was added carry a
    #shorter vector and keep the Mexican values of the Pars defaults, so old files
    #still load. Read here, ahead of the identical-technology block, so that block
    #still overrides them and picks up the new λf and αK. αN is derived from αMo and does
    #not update on its own when Pars is reconstructed, and neither do μo and νo from μ and ν,
    #nor αK and η from αMf
    if length(VEC)>=29
        par=Pars(par,αMo=VEC[28],λo=VEC[29],αN=1.0-VEC[28]-par.αLo)
    end
    if length(VEC)>=30
        par=Pars(par,λf=VEC[30])
    end
    if length(VEC)>=32
        par=Pars(par,μ=VEC[31],μo=VEC[32])
    end
    if length(VEC)>=34
        par=Pars(par,ν=VEC[33],νo=VEC[34])
    end
    if length(VEC)>=35
        #αK is the residual 1-αMf-αLf, the way αN is the residual for the oil sector, and
        #η=αLf/(αLf+αK)=αLf/(1-αMf) keeps the domestic intermediate sector's labor share
        #in value added equal to the final sector's, which is what η means
        par=Pars(par,αMf=VEC[35],αK=1.0-VEC[35]-par.αLf,η=par.αLf/(1.0-VEC[35]))
    end
    ...
    if VEC[14]==1.0
        ...
        λo=par.λf
        θo=VEC[20]
        par=Pars(par,αMo=αMo,αLo=αLo,αN=αN,λo=λo,θo=θo,μo=par.μ,νo=par.ν)
    else
        θo=VEC[26]
...
function UnpackModel_File(NAME::String,FOLDER::String)
    ...
    if FOLDER==" "
        ...
        VEC=readdlm(joinpath(FOLDER,NAME),',')
    end
...
function ValueAndBondsPayoff(nprime_ind::Int64,zprime::Float64,bprime::Float64,MODEL::Model)
    ...
    vd=itp_VD(bprime,zprime,nprime_ind)
    vp=itp_VP(bprime,zprime,nprime_ind)
    SDF=SDF_Lenders(par)
    if vd>vp
        #Default, lenders keep a defaulted claim until it is resolved
        @unpack itp_qD = SOLUTION
        return vd, SDF*itp_qD(bprime,zprime,nprime_ind)
    else
        @unpack γ = par
        if γ==1.0
            return vp, SDF
...
function DefaultedBondsPayoff(nprime_ind::Int64,zprime::Float64,b::Float64,MODEL::Model)
    #Payoff of one unit of face value of defaulted debt, evaluated at (z',n')
    #With probability φ the default is resolved and the unit becomes κ units of
    #a new bond, with probability 1-φ it is still a defaulted claim
    @unpack SOLUTION, par = MODEL
    @unpack φ, κ = par
    @unpack itp_qD = SOLUTION
    SDF=SDF_Lenders(par)
    vv, qq_resolved=ValueAndBondsPayoff(nprime_ind,zprime,κ*b,MODEL)
    return φ*κ*qq_resolved+(1.0-φ)*SDF*itp_qD(b,zprime,nprime_ind)
end

function UpdateExpectations!(MODEL::Model)
    @unpack GRIDS, par = MODEL
    @unpack PI_n, GR_n, GR_z, GR_b = GRIDS
    Nn=length(GR_n)
    #Update default
    #VD carries b now, build the interpolation over z once per (b,n') pair
    fill!(MODEL.SOLUTION.EVD,0.0)
    for b_ind in 1:length(GR_b)
        for nprime_ind in 1:Nn
            if sum(PI_n[:,nprime_ind])>0.0
                foo_mat=CreateInterpolation_ForExpectations(MODEL.SOLUTION.VD[b_ind,:,nprime_ind],GRIDS)
                for z_ind in 1:length(GR_z)
                    EVD=Expectation_over_zprime(foo_mat,z_ind,GRIDS)
                    for n_ind in 1:Nn
                        if PI_n[n_ind,nprime_ind]>0.0
                            MODEL.SOLUTION.EVD[b_ind,z_ind,n_ind]+=PI_n[n_ind,nprime_ind]*EVD
                        end
                    end
                end
            end
    ...
    #Update price of defaulted debt
    for I in CartesianIndices(MODEL.SOLUTION.qD)
        (b_ind,z_ind,n_ind)=Tuple(I)
        int_q=0.0
        b=GR_b[b_ind]
        for nprime_ind in 1:Nn
            if PI_n[n_ind,nprime_ind]>0.0
                foo_mat_qD(zprime::Float64)=DefaultedBondsPayoff(nprime_ind,zprime,b,MODEL)
                qDz=Expectation_over_zprime(foo_mat_qD,z_ind,GRIDS)
                int_q=int_q+PI_n[n_ind,nprime_ind]*qDz
            end
        end
        MODEL.SOLUTION.qD[I]=int_q
    end
    return nothing
end
...
function Evaluate_ValueFunction(x::State,I::CartesianIndex,bprime::Float64,MODEL::Model)
    ...
    if Default
                ...
                if SellOilDiscovery
                    if n_ind==2
                        ...
                        if n_ind==length(GRIDS.GR_n)
                            ...
                            @unpack OilRents_Discovery, KeepRentsInDefault = par
                            if KeepRentsInDefault
                                #Sold rents are given up only when the government repays
                                g=τ*GDP_Final_Matrix[z_ind,n_ind,1]+GDP_Oil_Matrix[z_ind,n_ind,1]
                            else
        ...
        #Default is resolved with probability φ, re-entry is with a stock of κ*b
        return Utility(cons,g,L,par)+β*φ*itp_EV(κ*b,z,n_ind)+β*(1.0-φ)*itp_EVD(b,z,n_ind)
    else
...
function Map_From_I_to_State(Default::Bool,I::CartesianIndex,MODEL::Model)
    #Default and repayment states share the same grid, b is carried through default
    @unpack GRIDS = MODEL
    @unpack GR_n, GR_z, GR_b = GRIDS
    (b_ind,z_ind,n_ind)=Tuple(I)
...
function RepaymentUpdater!(I::CartesianIndex,MODEL::Model)
    ...
    if MODEL.SOLUTION.VP[I]<MODEL.SOLUTION.VD[I]
        MODEL.SOLUTION.V[I]=MODEL.SOLUTION.VD[I]
    else
...
function UpdateDefault!(MODEL::Model)
    ...
    MODEL.SOLUTION.itp_VD=CreateInterpolation_ValueFunctions(MODEL.SOLUTION.VD,MODEL.GRIDS)
...
function UpdateRepayment!(MODEL::Model)
    ...
    MODEL.SOLUTION.itp_VP=CreateInterpolation_ValueFunctions(MODEL.SOLUTION.VP,MODEL.GRIDS)
    MODEL.SOLUTION.itp_V=CreateInterpolation_ValueFunctions(MODEL.SOLUTION.V,MODEL.GRIDS)
...
function UpdateSolution!(MODEL::Model)
    ...
    MODEL.SOLUTION.itp_EVD=CreateInterpolation_ValueFunctions(MODEL.SOLUTION.EVD,MODEL.GRIDS)
    MODEL.SOLUTION.itp_EV=CreateInterpolation_ValueFunctions(MODEL.SOLUTION.EV,MODEL.GRIDS)
    MODEL.SOLUTION.itp_q1=CreateInterpolation_Price(MODEL.SOLUTION.q1,MODEL.GRIDS)
    MODEL.SOLUTION.itp_qD=CreateInterpolation_Price(MODEL.SOLUTION.qD,MODEL.GRIDS)
...
function Quantities_Into_Paths!(t::Int64,x::State,bprime::Float64,PATHS::Paths,MODEL::Model)
    ...
    PATHS.Yo[t]=yo
    PATHS.L[t]=L
    PATHS.TB[t]=PATHS.GDP[t]-PATHS.C[t]-PATHS.G[t]
    PATHS.CA[t]=-(bprime-b)
...
function Simulate_EndogenousVariables!(Def0::Float64,B0::Float64,PATHS::Paths,MODEL::Model)
    ...
    for t in 1:T
        ...
        #Check if a default from last period is resolved this period
        Excluded=false
        if Def_1==1.0
            # if PATHS.Readmission[t]<=par.θ
            if rand()<=par.φ
                #Default is resolved, the stock is written down to κ*b
                b=par.κ*b
                PATHS.B[t]=b
            else
                Excluded=true
        ...
        if Excluded
            #Still in default, the stock stays outstanding
            Default=true
            x=State(Default,n_ind,n,z,b)
            bprime=b
            PATHS.Def[t]=1.0
            ...
            #In the market, either from repayment or just resolved, check default
            if itp_VD(b,z,n_ind)>itp_VP(b,z,n_ind)
                #Default
                Default=true
                x=State(Default,n_ind,n,z,b)
                bprime=b
    ...
    if Def_Ev>0.0
        for t in 2:length(PATHS_long.Def)
            if PATHS_long.Def[t]==1.0 && PATHS_long.Def[t-1]==0.0
                ...
                DROPAv=100*((PATHS_long.GDP[t]/gdpTrend)-1)
                drpAv_DefEv=drpAv_DefEv+DROPAv/Def_Ev
            end
        end
    end
    MOM.GDP_drop_DefEv=drp_DefEv
    MOM.GDP_dropAv_DefEv=drpAv_DefEv
...
function Moments_To_RowVector(MOM::Moments)
    ...
    VEC=hcat(VEC,MOM.GDP_dropAv_DefEv)
...
function RelevantTargets()
    # ρ_GDP=0.579
    σ_GDP=3.11
    GDP_dropAv_DefEv=-13.28   #Deviation from trend, matched with GDP_dropAv_DefEv
    DefaultPr=0.51
    MeanSpreads=2.9
    WK_GDP=8.09
    return vcat(σ_GDP,GDP_dropAv_DefEv,MeanSpreads,WK_GDP)
end
...
function RelevantMatricesFromCase(case_column::Int64,SETUP_FILE::String,FOLDER_TRIES::String)
    ...
    if FOLDER_TRIES==" "
        ...
        MOMENT_MATCHING_FILE_NAME=joinpath(FOLDER_TRIES,"TriedCalibrations, $CASE_NAME.csv")
    end
    ...
    col_GDP_dropAv_DefEv=19
    col_MeanSpreads=6
    col_DefaultPr=5
    col_WK_GDP=20
    COLUMNS=vcat(col_σ_GDP,col_GDP_dropAv_DefEv,col_MeanSpreads,col_WK_GDP)

    MAT_PARS=1.0*XX_MOMENTS[2:end,1:4]
    σ_GDP=1.0*XX_MOMENTS[2:end,col_σ_GDP]
    GDP_dropAv_DefEv=1.0*XX_MOMENTS[2:end,col_GDP_dropAv_DefEv]
    ...
    MAT_RELEVANT=hcat(σ_GDP,GDP_dropAv_DefEv,MeanSpreads,WK_GDP)
...
function Set_Nb_Setup_Calibrated(Nb_new::Int64)
    CALIBRATED_SETUP_FILE_NAME="Setup_Calibrated.csv"
    MAT=readdlm(CALIBRATED_SETUP_FILE_NAME,',')
    row_Nb=findfirst(MAT[:,1] .== "Nb")
    for case_column in 4:size(MAT,2)
        MAT[row_Nb,case_column]=Nb_new
    end
    writedlm(CALIBRATED_SETUP_FILE_NAME,MAT,',')
    return nothing
end
...
function Model_Sell_Giant_Field(setup_coulumn::Int64,SETUP_FILE::String;
                                WithWindfall::Bool=true,KeepRentsInDefault::Bool=false)
    #The two keywords turn the sell counterfactual's two budget changes on one at a time,
    #for the welfare decomposition of Subsection 4.2. Their defaults are the
    #paper's sell economy V^S. WithWindfall=false is V^F, and adding KeepRentsInDefault
    #is V^R
    XX=readdlm(SETUP_FILE,',')
    NAME=convert(String,XX[1,setup_coulumn])
    VEC_PAR=XX[2:end,setup_coulumn]*1.0
    ...
    par_Sell=Pars(par,SellOilDiscovery=true,OilRents_Discovery=OilRents_Discovery,NPV_Discovery=NPV_Discovery,
                  KeepRentsInDefault=KeepRentsInDefault)
...
function Welfare_Gains_t_Change_Model(AtDiscovery::Bool,t::Int64,TS::Paths,MOD0::Model,MOD1::Model)
    ...
    if Def==1.0
        if AtDiscovery
            v0=MOD0.SOLUTION.itp_VD(b,z,2)
            v1=MOD1.SOLUTION.itp_VD(b,z,2)
        else
            v0=MOD0.SOLUTION.itp_VD(b,z,n_ind)
            v1=MOD1.SOLUTION.itp_VD(b,z,n_ind)
        end
    else
        if AtDiscovery
...
function Value_At_Discovery_t(t::Int64,TS::Paths,MOD::Model)
    #Value in the period of a hypothetical discovery, at the state drawn at t.
    #TS only has periods with no discovery, so every period is a draw from the
    #ergodic distribution, as in Welfare_Gains_t
    @unpack SOLUTION = MOD
    z=TS.z[t]; b=TS.B[t]; Def=TS.Def[t]
    if Def==1.0
        return SOLUTION.itp_VD(b,z,2)
    else
        return SOLUTION.itp_V(b,z,2)
    end
end

function Average_Welfare_Decomposition(N::Int64,MOD_K::Model,MOD_R::Model,
                                       MOD_F::Model,MOD_S::Model)
    #Decomposition of the gains from selling the giant field's rents, following
    #Aguiar, Amador and Fourakis (2020). The N discoveries are drawn from the ergodic
    #distribution of the benchmark economy and all four value functions are evaluated
    #at the same states, so the three factors multiply to the total draw by draw.
    #Returns the four averaged terms in percent, the product of the three factors,
    #and the largest violation of the identity across draws
    @unpack par = MOD_K
    @unpack σ, drp = par
    T=drp+N; ForMoments=true
    TS=Simulate_Paths(ForMoments,T,MOD_K)
    ζR=0.0; ζD=0.0; ζW=0.0; ζ=0.0; MaxResidual=0.0
    for i in 1:N
        t=drp+i
        vK=Value_At_Discovery_t(t,TS,MOD_K)
        vR=Value_At_Discovery_t(t,TS,MOD_R)
        vF=Value_At_Discovery_t(t,TS,MOD_F)
        vS=Value_At_Discovery_t(t,TS,MOD_S)
        fR=(vR/vK)^(1/(1-σ))
        fD=(vF/vR)^(1/(1-σ))
        fW=(vS/vF)^(1/(1-σ))
        ff=(vS/vK)^(1/(1-σ))
        MaxResidual=max(MaxResidual,abs(fR*fD*fW-ff))
        ζR=ζR+100*(fR-1)/N
        ζD=ζD+100*(fD-1)/N
        ζW=ζW+100*(fW-1)/N
        ζ=ζ+100*(ff-1)/N
    end
    #Product of the three averaged factors, which differs from the averaged total only
    #by the covariance of the factors across draws
    ζProduct=100*(((1+ζR/100)*(1+ζD/100)*(1+ζW/100))-1)
